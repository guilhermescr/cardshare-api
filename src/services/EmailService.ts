...
export async function sendEmail(
  ...
  await transporter.sendMail({
    from: `"Cards API" <${process.env.SMTP_USER}>`,
    to,
    subject,
    html,
  });
  ...
  return `
    <div style="font-family: Arial, sans-serif; background: #f9f9f9; padding: 32px;">
      <div style="max-width: 480px; margin: auto; background: #fff; border-radius: 8px; box-shadow: 0 2px 8px #eee; padding: 24px;">
        <h2 style="color: #2d7ff9;">Welcome to Cards API, ${username}!</h2>
        ...
        <p style="font-size: 12px; color: #bbb;">Cards API &copy; ${new Date().getFullYear()}</p>
      </div>
    </div>
  `;
}
